 ...
 * Jan 20. 2008     bsigner     Adapted to new BasicDialog functionality
 *
 * -----------------------------------------------------------------------
 *
...
public class OptionsDialog extends BasicDialog {
   ...
   private static String OPTIONS_DIALOG_KEY = "OptionsDialog";
   ...
   private void init() {
      ...
      this.add(tabbedPane, new GridBagConstraints(0, 0, 1, 1, 1, 1,
   ...
   private void addFirstTab() {
      ...
      List<String> devices = view.getModel().getGestureConfiguration()
            .getInputDevices();
   ...
   private void addSecondTab() {
      ...
      // Creates the radio buttons.
      aPanel.add(startupBox, new GridBagConstraints(0, 0, 1, 1, 1, 1,
   ...
   private class OkListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         boolean needUpdate = (!newSelDevice.equals(configuration
               .getInputDeviceName()) || configuration.getMinimize() != startupBox
               .isSelected());
         ...
         if (needUpdate) {
            ...
            try {
               confFile = new File(ClassLoader.getSystemResource(
                     Geco.GECO_CONFIGURATION).toURI());
            }
            catch (URISyntaxException ex) {
               confFile = new File(ClassLoader.getSystemResource(
                     Geco.GECO_CONFIGURATION).getPath());
            }
            XMLGeco.exportGestureConfiguration(confFile, devices, arr,
                  startupBox.isSelected(), view.getModel().getProjectFile()
                        .getPath());

            view.getModel().resetInputDevice();
            view.getModel().setGestureConfiguration(
                  new Configuration(Geco.GECO_CONFIGURATION));
