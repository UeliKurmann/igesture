...
public class RubineAlgorithm extends SampleBasedAlgorithm {
	...
	static {
		 ...
		 */
		DEFAULT_CONFIGURATION.put(Config.MIN_DISTANCE.name(),
				DEFAULT_MIN_DISTANCE);
		DEFAULT_CONFIGURATION.put(Config.MAHALANOBIS_DISTANCE.name(),
				DEFAULT_MAHALANOBIS_DISTANCE);
		DEFAULT_CONFIGURATION.put(Config.PROBABILITY.name(),
				DEFAULT_PROBABILITY);
		DEFAULT_CONFIGURATION.put(Config.FEATURE_LIST.name(), F1.class
				.getName()
				+ Constant.COMMA
				+ F2.class.getName()
				+ Constant.COMMA
	...
	/**
	 * Computes the feature vector for a given sample. The features to compute
	 * are passed as an array. The method returns a double vector representing
	 * the feature vector.
	 * 
	 * @param sample
	 *            the sample.
	 ...
	 * 
	 * @param gestureClass
	 *            the gesture class to compute the covariance matrix.
	...
	private void computeWeights() throws AlgorithmException {
		...
		for (final GestureClass gestureClass : gestureSet.getGestureClasses()) {
			final DoubleVector meanVector = meanFeatureVector.get(gestureClass);
			final DoubleVector weightVector = new DoubleVector(meanVector
					.size());
	...
	private ResultSet classify(DoubleVector inputFeatureVector) {
		...
		for (final GestureClass currentGestureClass : gestureSet
			...
			LOGGER.info(currentGestureClass.getName() + Constant.COLON_BLANK
					+ v);
	 ...
	 * 
	 * @param gestureClass
	 *            the gesture class to be used.
	 * @param inputVector
	 *            the input vector the distance has to be computed for.
