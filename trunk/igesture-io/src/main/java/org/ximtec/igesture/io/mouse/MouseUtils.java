	...
	public MouseUtils() {
		...
		msListener = new MouseProc() {
			public LRESULT callback(int code, WPARAM wParam, LPARAM lParam) {
				System.out.println("mouse callback code: " +code + " wParam "+ wParam + " lParam "+lParam );
				//getCoordinates!!
		...
		handle = User32.INSTANCE.SetWindowsHookExW(User32.WH_MOUSE_LL, msListener, hinst, Kernel32.INSTANCE.GetCurrentThreadId());
