...
public class TestSetPanel extends AbstractPanel {
   ...
   private void initSampleSection(TestSet testSet) {
      ...
      builder.append(ComponentFactory.createLabel(GestureConstants.TESTSET_NAME));
      ...
      builder.append(new JLabel("Descriptor has " + testSet.getSamples().size() + " samples."),3);
      builder.nextLine(4);

      for (final Gesture<?> sample : testSet.getSamples()) {
         // TODO: Remove unsafe cast!
         final JLabel label = new JLabel(new ImageIcon(GestureTool.createNoteImage((Note)sample.getGesture(), 100, 100)));
         ...
         label.addMouseListener(new MouseAdapter(){
            ...
            private void popUp(MouseEvent e){
               if(e.isPopupTrigger()){
                  ...
                  item.setAction(new RemoveSampleAction(TestSetPanel.this.testSet, sample));
                  menu.add(item);
   ...
   private void initInputSection(TestSet descriptor) {
      ...
      buttonPanel.add(new BasicButton(new AddSampleAction(descriptor)));
