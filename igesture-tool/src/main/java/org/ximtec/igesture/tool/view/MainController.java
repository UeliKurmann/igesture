...
public class MainController extends DefaultController implements Service {
   ...
   private Controller createController(Class< ? > controllerClass)
      ...
      try {
         if (!instantiated
               && (controllerClass.getConstructor(Controller.class) != null)) {
   ...
   protected void execChangeTab() {
      ...
      /*
       * Only listeners for the current active tab are allowed. This way the
       * performed gestures are only drawn on the current tab. On change of tab
       * remove all listeners.
       */
      for (AbstractGestureDevice< ? , ? > device : getLocator().getService(
            DeviceManagerService.IDENTIFIER, DeviceManagerController.class)
            .getDevices()) {
   ...
   protected void execShowAboutDialog() {
      ...
      AboutDialog dialog = new AboutDialog(GestureConstants.ABOUT, getLocator()
   ...
   private void initServices() {
      ...
      try {
         properties
               .loadFromXML(new FileInputStream(GestureConstants.PROPERTIES));
      }
      catch (Exception e) {
      ...
      // init device manager service, ! guiBundle service needs to be
      // installed first
      deviceManager = new DeviceManagerService(this,
            GestureConstants.DEVICE_MANAGER, guiBundle);
    ...
    * 
    * @param controllers An array of controllers. These controllers are
    *           initialised.
    */
    ...
    * 
    * @param key the key used in the Gui Bundle
